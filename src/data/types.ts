...
export interface PledgeReviewItem {
  ...
  inProgress?: string;
  /**
   * 타·과거 후보 공약/표준 의제와 중복한 정황 + 근거(주의). 없으면 생략.
   * 방향(원조-모방)은 시점이 명확히 앞선 원본(전임·현직 임기 사업, 타 지자체 기존 모델, 과거·역대 의제, 국가 법·계획)이 있을 때만 명시한다.
   * 같은 선거 동시 출마자 간 중복은 제출 시점이 사실상 동일해 선후를 입증할 수 없으므로 ‘누가 베꼈다’고 단정하지 않고,
   * 공통 출처(국가계획·법·전임 정책 등)를 근거로 중립 표기한다. 근거는 가급적 원본·시점·확인경로를 함께 적는다.
   */
  recycled?: string;
